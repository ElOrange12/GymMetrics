...
$nombre_col        = ($lang_code === 'en') ? 'COALESCE(e.nombre_en, e.nombre)' : 'e.nombre';      // Para JOINs
$nombre_col_direct = ($lang_code === 'en') ? 'COALESCE(nombre_en, nombre)' : 'nombre';            // Para query directa
...
try {
    ...
    $stmtEj = $pdo->prepare("SELECT dr.id as detalle_id, dr.ejercicio_id, dr.orden, {$nombre_col} AS nombre FROM detalles_rutina dr JOIN ejercicios e ON dr.ejercicio_id = e.id WHERE dr.rutina_id = ? ORDER BY dr.orden");
    $stmtEj->execute([$rutina_id]);
    $ejercicios_actuales = $stmtEj->fetchAll(PDO::FETCH_ASSOC);

    foreach ($ejercicios_actuales as &$ej) {
        $stmtSer = $pdo->prepare("SELECT id, numero_serie, reps_objetivo, peso_objetivo FROM rutina_series WHERE detalle_rutina_id = ? ORDER BY numero_serie");
        $stmtSer->execute([$ej['detalle_id']]);
        $ej['series'] = $stmtSer->fetchAll(PDO::FETCH_ASSOC);
    }

    $stmtM = $pdo->query("SELECT id, {$nombre_col_direct} AS nombre FROM ejercicios ORDER BY nombre ASC");
    $ejercicios_maestros = $stmtM->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title><?= $lang['edit_structure'] ?> | GymMetrics</title>
...
<body>
    <nav class="navbar">
        ...
        <span style="font-weight:bold;font-size:18px"><?= $lang['edit_structure'] ?></span>
    </nav>
    <div class="edit-container">
        <form action="controladores/procesaredicion.php" method="POST" id="formEdicion">
            ...
            <div class="form-group">
                <label class="form-label"><?= $lang['routine_name_label'] ?> (<?= $rutina['dia_semana'] ?>)</label>
                    ...
                    <div class="exercise-block">
                        <div style="display:flex;justify-content:space-between;margin-bottom:15px">
                            <select name="ejercicios[<?= $index ?>]" class="form-select" required style="width:85%">
                                <option value=""><?= $lang['select_exercise'] ?></option>
                        ...
                        <div id="series-container-<?= $index ?>">
                            <?php foreach ($ej['series'] as $sIndex => $serie): ?>
                                <div class="serie-row">
                                    ...
                                    <input type="number" name="reps[<?= $index ?>][]" class="form-input" value="<?= $serie['reps_objetivo'] ?>" placeholder="<?= $lang['reps'] ?>" required>
                        ...
                        <button type="button" class="btn-small-add" style="border-style:solid;font-size:11px;padding:5px;margin-top:10px" onclick="addSerie(<?= $index ?>)">
                            <i class="fa-solid fa-plus"></i> <?= $lang['add_series'] ?>
                        </button>
                    </div>
                <?php endforeach; ?>
            </div>
    ...
    <script>
        ...
        const txtSelExer = <?= json_encode($lang['select_exercise']) ?>;
        const opcionesEjercicios = `
            <option value="">${txtSelExer}</option>
            <?php foreach($ejercicios_maestros as $ej): ?>
                <option value="<?= $ej['id'] ?>"><?= htmlspecialchars(addslashes($ej['nombre'])) ?></option>
            <?php endforeach; ?>
        `;
        function addExerciseField(){const div=document.createElement('div');div.className='exercise-block';div.innerHTML=`<div style="display:flex;justify-content:space-between;margin-bottom:15px"><select name="ejercicios[${exerciseIndex}]" class="form-select" required style="width:85%">${opcionesEjercicios}</select><i class="fa-solid fa-trash text-muted" onclick="this.parentElement.parentElement.remove()" style="cursor:pointer;padding:10px"></i></div><div id="series-container-${exerciseIndex}">${generarFilaSerie(exerciseIndex,1)}</div><button type="button" class="btn-small-add" style="border-style:solid;font-size:11px;padding:5px;margin-top:10px" onclick="addSerie(${exerciseIndex})"><i class="fa-solid fa-plus"></i> ${txtAddSet}</button>`;container.appendChild(div);exerciseIndex++}
        function addSerie(id){const sc=document.getElementById(`series-container-${id}`);const n=sc.children.length+1;const td=document.createElement('div');td.innerHTML=generarFilaSerie(id,n);sc.appendChild(td.firstElementChild)}
        function generarFilaSerie(id,n){return`<div class="serie-row"><span style="color:var(--text-muted);font-size:12px;font-weight:bold;text-align:center">S${n}</span><input type="number" name="reps[${id}][]" class="form-input" placeholder="${txtReps}" required><input type="hidden" name="pesos[${id}][]" value="0"><i class="fa-solid fa-xmark text-muted" onclick="this.parentElement.remove()" style="cursor:pointer;text-align:center"></i></div>`}
    </script>
</body>
</html>