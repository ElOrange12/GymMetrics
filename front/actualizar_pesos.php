...
<head>
    ...
    <title><?= $lang['update_weights'] ?> | GymMetrics</title>
...
<body>
    <nav class="navbar">
        ...
        <span style="font-weight:bold;font-size:18px"><?= $lang['update_weights'] ?></span>
    </nav>
    <div class="edit-container">
        <h3 style="text-align:center;color:white;margin-bottom:20px"><?= htmlspecialchars($rutina['nombre_rutina']) ?></h3>
        <form action="controladores/procesarpesos.php" method="POST">
                ...
                <div class="ejercicio-bloque">
                    ...
                    <?php foreach ($ej['series'] as $serie): ?>
                        <div class="serie-fila">
                            <div class="serie-info">S<?= $serie['numero_serie'] ?>: <strong><?= $serie['reps_objetivo'] ?> <?= $lang['reps'] ?></strong></div>
            ...
            <button type="submit" class="btn-save" style="margin-top:20px;background:#27ae60">
                <i class="fa-solid fa-check"></i> <?= $lang['save_weights'] ?>
            </button>
        </form>
    </div>
</body>
