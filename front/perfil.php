...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($nuevo_nombre) && !empty($nuevo_email)) {
        try {
            ...
            // Mensajes también podrían ir al diccionario, pero los dejamos estáticos de momento para no complicar
            $mensaje = '<div class="alert success"><i class="fa-solid fa-check-circle"></i> Perfil actualizado correctamente.</div>';
        } catch (PDOException $e) {
            $mensaje = '<div class="alert error"><i class="fa-solid fa-triangle-exclamation"></i> Error al actualizar.</div>';
        }
    }
}
...
try {
    ...
    $fecha_registro = !empty($user['fecha_registro']) ? date('d/m/Y', strtotime($user['fecha_registro'])) : '-';
} catch (PDOException $e) {
...
<head>
    ...
    <title><?= $lang['my_profile'] ?> | GymMetrics</title>
    ...
    <style>
        ...
        .profile-avatar-large { font-size: 70px; color: var(--blue-neon, #3498db); background: #151b22; width: 120px; height: 120px; line-height: 120px; border-radius: 50%; margin: 0 auto 15px auto; border: 2px solid #34495e; }
        ...
        .form-control { width: 100%; padding: 12px 15px; border-radius: 8px; border: 1px solid #34495e; background: #0f141a; color: white; box-sizing: border-box; }
        .form-control:focus { outline: none; border-color: var(--blue-neon, #3498db); }
        .btn-save { background: var(--blue-neon, #3498db); color: white; border: none; padding: 15px; width: 100%; border-radius: 8px; font-weight: bold; cursor: pointer; }
...
<body>
    ...
    <nav class="navbar">
        ...
        <span style="font-weight: bold; font-size: 18px; letter-spacing: 1px;"><?= $lang['my_profile'] ?></span>
    </nav>
    ...
    <div class="profile-container">
        ...
        <div class="profile-card">
            <h3><i class="fa-solid fa-id-card"></i> <?= $lang['personal_data'] ?></h3>
            <form action="perfil.php" method="POST">
                <div class="form-group">
                    <label for="nombre_usuario"><?= $lang['username'] ?></label>
                ...
                <button type="submit" class="btn-save">
                    <i class="fa-solid fa-floppy-disk"></i> <?= $lang['save_changes'] ?>
                </button>
            </form>
        </div>

        <div class="profile-card">
            <h3><i class="fa-solid fa-chart-pie"></i> <?= $lang['account_stats'] ?></h3>
            <div class="stats-row">
                <div><?= $lang['member_since'] ?></div>
    ...
    <nav class="bottom-nav" style="position: fixed; bottom: 0; left: 0; width: 100%; background: #0f141a; border-top: 1px solid #34495e; display: flex; justify-content: space-around; padding: 15px 0; z-index: 1000;">
        <a href="exito.php" class="nav-item" style="color: var(--text-muted); text-decoration: none; display: flex; flex-direction: column; align-items: center; gap: 5px; font-size: 12px; width: 33%;"><i class="fa-solid fa-house" style="font-size: 20px;"></i><span><?= $lang['nav_home'] ?></span></a>
        <a href="rutinas.php" class="nav-item" style="color: var(--text-muted); text-decoration: none; display: flex; flex-direction: column; align-items: center; gap: 5px; font-size: 12px; width: 33%;"><i class="fa-solid fa-book-journal-whills" style="font-size: 20px;"></i><span><?= $lang['nav_routines'] ?></span></a>
        <a href="perfil.php" class="nav-item" style="color: var(--blue-neon, #3498db); text-decoration: none; display: flex; flex-direction: column; align-items: center; gap: 5px; font-size: 12px; width: 33%;"><i class="fa-solid fa-gear" style="font-size: 20px;"></i><span><?= $lang['nav_settings'] ?></span></a>
    </nav>
</body>
</html>
