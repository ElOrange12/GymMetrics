...
// Mapa de grupos musculares ES → clave del $lang para mostrar traducido
$grupos_map = [
    'Pecho'   => 'muscle_chest',
    'Espalda' => 'muscle_back',
    'Pierna'  => 'muscle_legs',
    'Hombro'  => 'muscle_shoulders',
    'Brazos'  => 'muscle_arms',
    'Core'    => 'muscle_core',
];
...
try {
    ...
    if ($rol !== 'admin') { header("Location: exito.php"); exit(); }
    ...
    $lista_ejercicios = $pdo->query("SELECT id, nombre, nombre_en, grupo_muscular FROM ejercicios ORDER BY grupo_muscular, nombre")->fetchAll(PDO::FETCH_ASSOC);

} catch (PDOException $e) { die("Error crítico: " . $e->getMessage()); }
?>
<!DOCTYPE html>
<html lang="<?= $lang_code ?>">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= $lang['admin_panel'] ?> | GymMetrics</title>
    ...
    <style>
        ...
        .form-add{display:flex;gap:10px;margin-top:15px;align-items:center;flex-wrap:wrap}
        .form-add input,.form-add select{padding:10px;background:#0f141a;border:1px solid #34495e;color:white;border-radius:5px;flex:1;min-width:120px}
        .btn-green{background:#2ecc71;color:white;border:none;padding:10px 20px;border-radius:5px;cursor:pointer;font-weight:bold}
        .tag-en{font-size:11px;color:#95a5a6;display:block;margin-top:2px}
    </style>
</head>
<body>
    <nav class="navbar">
        <div class="nav-brand" style="color:#e74c3c"><i class="fa-solid fa-unlock-keyhole"></i> ADMIN</div>
        <a href="exito.php" style="color:white;text-decoration:none;font-size:14px;border:1px solid #34495e;padding:5px 15px;border-radius:20px">
            <i class="fa-solid fa-arrow-left"></i> <?= $lang['admin_back'] ?>
        </a>
    </nav>
    <div class="admin-wrap">
        <h2><?= $lang['admin_panel'] ?></h2>
        <p style="color:var(--text-muted);margin-bottom:25px"><?= $lang['admin_subtitle'] ?></p>

        <div class="grid-stats">
            <div class="stat-box">
                <i class="fa-solid fa-users"></i>
                <div class="num"><?= $total_usuarios ?></div>
                <div style="color:var(--text-muted);font-size:12px"><?= $lang['admin_users_label'] ?></div>
            </div>
            <div class="stat-box">
                <i class="fa-solid fa-trophy"></i>
                <div class="num"><?= $total_entrenos ?></div>
                <div style="color:var(--text-muted);font-size:12px"><?= $lang['admin_completed_workouts'] ?></div>
            </div>
            <div class="stat-box">
                <i class="fa-solid fa-dumbbell"></i>
                <div class="num"><?= count($lista_ejercicios) ?></div>
                <div style="color:var(--text-muted);font-size:12px"><?= $lang['admin_exercises_db'] ?></div>
            </div>
        </div>

        <!-- Gestión de Usuarios -->
        <div class="admin-section">
            <h3><i class="fa-solid fa-users-gear"></i> <?= $lang['admin_users_management'] ?></h3>
            <div style="overflow-x:auto">
                <table class="admin-table">
                    <tr>
                        <th><?= $lang['admin_col_id'] ?></th>
                        <th><?= $lang['admin_col_user'] ?></th>
                        <th><?= $lang['admin_col_email'] ?></th>
                        <th><?= $lang['admin_col_role'] ?></th>
                        <th><?= $lang['admin_col_action'] ?></th>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td style="color:<?= $u['rol']=='admin' ? '#e74c3c' : '#2ecc71' ?>;text-transform:uppercase;font-size:11px;font-weight:bold"><?= $u['rol'] ?></td>
                        <td>
                            <?php if($u['id'] != $user_id): ?>
                            <form action="controladores/admin_procesar.php" method="POST" style="margin:0" onsubmit="return confirm('<?= $lang['admin_delete_user_confirm'] ?>')">
        ...
        <div class="admin-section">
            <h3><i class="fa-solid fa-dumbbell"></i> <?= $lang['admin_exercise_catalog'] ?></h3>
            <form action="controladores/admin_procesar.php" method="POST" class="form-add">
                ...
                <input type="text" name="nombre_ejercicio" placeholder="<?= $lang['admin_new_exercise_es'] ?>" required>
                <input type="text" name="nombre_ejercicio_en" placeholder="<?= $lang['admin_new_exercise_en'] ?>">
                <select name="grupo_muscular" required>
                    <?php foreach($grupos_map as $val => $key): ?>
                        <option value="<?= $val ?>"><?= $lang[$key] ?></option>
                    <?php endforeach; ?>
                </select>
                <button type="submit" class="btn-green"><i class="fa-solid fa-plus"></i> <?= $lang['admin_add'] ?></button>
            </form>
            <div style="max-height:400px;overflow-y:auto;margin-top:20px;border-top:1px solid #34495e;padding-top:10px">
                <table class="admin-table">
                    <tr>
                        <th><?= $lang['admin_col_id'] ?></th>
                        <th><?= $lang['admin_col_name'] ?></th>
                        <th><?= $lang['admin_col_name_en'] ?></th>
                        <th><?= $lang['admin_col_group'] ?></th>
                        <th><?= $lang['admin_col_action'] ?></th>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td style="color:#95a5a6;font-size:13px"><?= htmlspecialchars($ej['nombre_en'] ?? '—') ?></td>
                        <td><?= htmlspecialchars($lang[$grupos_map[$ej['grupo_muscular']] ?? 'muscle_core']) ?></td>
                        <td>
                            <form action="controladores/admin_procesar.php" method="POST" style="margin:0" onsubmit="return confirm('<?= $lang['admin_delete_exercise_confirm'] ?>')">
